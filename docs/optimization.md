...
### 23. 인스타그램 최신 피드 3x2 반응형 갤러리 섹션 구축 (`src/components/InstagramFeedSection.jsx`, `src/pages/Home.jsx`, `src/index.css`)
- **작업 브랜치**: `feature/instagram-feed`
- **개선 내역**:
  - 메인 페이지 **[이벤트 & 매장 소식] 섹션 바로 다음**에 인스타그램 소식 피드 섹션 추가.
  - **3열 x 2행 (총 6개 게시물)** 갤러리 그리드로 렌더링 및 모바일/태블릿 반응형 전환 적용.
  - 마우스 호버 시 인스타그램 아이콘, 캡션 요약, 좋아요/댓글 수 오버레이 애니메이션 및 `@geummakchang` 계정 연결 링킹 구현.
