...
### 21. 창업 문의 관리 더미/허수 문의 데이터 100% 완전 삭제 및 초기화 기능 추가 (`src/components/admin/CustomerTab.jsx`)
- **개선**:
  - 브라우저 로컬 캐시(`localStorage`) 및 Supabase 테이블에 남아있을 수 있는 초기 샘플 허수 창업 문의 데이터(6건)를 100% 삭제 및 소거.
  - 관리자 페이지 `[창업 문의 관리]` 상단에 **`[문의 전체 삭제]`** 버튼을 추가하여, 언제든지 원클릭으로 초기화 및 정제가 가능하도록 보완.
